...
namespace addons\tags\rus_hybrid_auth\lib\Hybrid\Providers;
use Hybrid_Auth;
...
class Hybrid_Providers_Vkontakte extends Hybrid_Provider_Model_OAuth2 {
    ...
    public $fields = [// Old that saved for backward-compability
        'identifier' => 'id', 'firstName' => 'first_name', 'lastName' => 'last_name', 'displayName' => 'screen_name', 'gender' => 'sex', 'photoURL' => 'photo_big', 'home_town' => 'home_town', 'profileURL' => 'domain',      // Will be converted in getUserByResponse()
        // New
        'nickname' => 'nickname', 'bdate' => 'bdate', 'timezone' => 'timezone', 'photo_rec' => 'photo_rec', 'domain' => 'domain', 'photo_max' => 'photo_max_orig', 'home_phone' => 'home_phone', 'city' => 'city',        // Will be converted in getUserByResponse()
    ...
    function getUserByResponse($response, $withAdditionalRequests = false) {
        ...
        if(!empty($user -> city) && $withAdditionalRequests) {
            $params = ['v' => $this -> version, 'city_ids' => $user -> city];
            ...
            if(is_object($city) || is_string($city)) {
                $user -> city = isset($city -> title) ? $city -> title : null;
            }
        }
