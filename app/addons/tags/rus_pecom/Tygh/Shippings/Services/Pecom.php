...
class Pecom implements IService
{
    ...
    private function _getRates($response)
    {
        ...
        if (!empty($cost)) {
            if (
                $shipping_info['take'] === YesNo::YES
                && !empty($response['take'])
            ) {
                $cost += array_sum($response['take']);
            }

            if (
                $shipping_info['deliver'] === YesNo::YES
                && !empty($response['deliver'])
            ) {
                $cost += array_sum($response['deliver']);
            }

            if (
                $shipping_info['package_hard'] === YesNo::YES
                && !empty($response['ADD'])
            ) {
                $cost += array_sum($response['ADD']);
            }

            if (
                $shipping_info['pal'] === YesNo::YES
                && !empty($response['ADD_2'])
            ) {
                $cost += array_sum($response['ADD_2']);
            }

            if (
                $shipping_info['insurance'] === YesNo::YES
                && !empty($response['ADD_3'])
            ) {
                $cost += array_sum($response['ADD_3']);
            }

            if (!empty($response['alma_auto'])) {
                $cost += array_sum($response['alma_auto']);
            }
        }
