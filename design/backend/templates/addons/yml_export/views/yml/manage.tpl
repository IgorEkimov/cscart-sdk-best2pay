                        ...
                        <tbody class="cm-hide-inputs">
                            ...
                            >
                                ...
                                <td class="{$no_hide_input}">
                                    <a class="row-status link--monochrome" href="{"yml.update?price_id=`$price.param_id`"|fn_url}">{$price.param_data.name_price_list}</a>
                                    {if $price.company_id}{include file="views/companies/components/company_name.tpl" object=$price}{/if}
                                </td>
                                <td class="{$no_hide_input}">
                                    <a class="row-status link--monochrome" href="{$price.generate_link}" target="_blank">{__("yml_export.create")}</a>
                                </td>
                                <td class="{$no_hide_input}">
                                    <a class="row-status link--monochrome" href="{$price.get_link}" target="_blank">{__("yml_export.go_to_link")}</a>
                                </td>
                                ...
                                <td class="right">
                                    {include file="common/select_popup.tpl" type="yml_param" id=$price.param_id status=$price.status non_editable=!$allow_save hidden=true object_id_name="param_id" table="yml_param" popup_additional_class="`$no_hide_input` dropleft" hidden=false}
                                </td>
                            </tr>
                            {/foreach}
                        </tbody>
...
{capture name="adv_buttons"}
    {include file="common/tools.tpl"
        tool_href="yml.update"
        tool_override_meta="btn btn-primary"
        prefix="top"
        hide_tools="true"
        title=__("yml_export.add_price")
        link_text=__("yml_export.add_price")
        icon="icon-plus"
    }
{/capture}
